...
var Remus = function(opts) {
  ...
  if(supressRedisError) {
    ...
    this.incomingClient.on('ready', function() {
      if(self._reconnecting) {
        self.incomingClient.punsubscribe(self._messagePattern);
        self.incomingClient.punsubscribe(self._broadcastPattern);
        self.incomingClient.psubscribe(self._messagePattern);
        self.incomingClient.psubscribe(self._broadcastPattern);
        for(var r in self._rooms) {
          self.incomingClient.punsubscribe(self._rooms[r]);
          self.incomingClient.psubscribe(self._rooms[r]);
        }
  ...
  /**
   * Associative array of the rooms currently listening to.
   * @private
   * @type {Object}
   */
  this._rooms = {};
  ...
  /**
   * Whether remus is currently reconnecting to redis.
   * @type {Boolean}
   */
...
Remus.prototype.sendMessage = function(recepientId, msg, callback) {
  ...
  if(callback || typeof callback === Function) {
    ...
    var msgTimeout = setTimeout(function() {
      delete self._messagesWaiting[msgId];
...
Remus.prototype.listenToRoom = function(room, callback) {
  ...
  if(!this._rooms.hasOwnProperty(roomChannel)) {
    this.incomingClient.psubscribe(roomChannel);
  }

  this._rooms[roomChannel] = true;
...
Remus.prototype.ignoreRoom = function(room, callback) {
  ...
  delete this._rooms[roomChannel];

  if(typeof callback === 'Function') {
    return this.removeListener('room:' + room, callback);
  }
  return this.removeAllListeners('room:' + room);
};
...
Remus.prototype.handlePMessage = function(pattern, channel, msg) {
  ...
  if(pattern.indexOf(roomMessagePrefix) === 0) {
